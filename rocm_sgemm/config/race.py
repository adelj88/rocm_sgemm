...
# Canonical tunable-parameter order. Must match tune.py's PARAM_ORDER and the argument order the
# C++ tuner binary expects: M N K <these params in order> layout_a layout_b layout_c gpu_arch.
PARAM_ORDER = [
    'warps_m', 'warps_n',
...
def evaluate_config_once(M, N, K, la, lb, lc, config, gpu_arch):
    ...
    # config could be a dict or a tuple (in PARAM_ORDER). Handle both.
    d = config if isinstance(config, dict) else dict(zip(PARAM_ORDER, config))

    try:
        result = subprocess.run([
            "./rocm_sgemm/tuner",
            str(M), str(N), str(K),
            *[str(d[name]) for name in PARAM_ORDER],
...
def tuple_to_dict(config_tuple):
    return dict(zip(PARAM_ORDER, config_tuple))
...
def main():
    p = argparse.ArgumentParser(description='SGEMM Config Racing Tool')
